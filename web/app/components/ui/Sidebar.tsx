...
export default function Sidebar() {
  ...
  const navItems = [
    ...
    { label: "Sales", href: "/dashboard/sales", icon: <ShoppingCart size={20} /> },
    { label: "Expenses", href: "/dashboard/expenses", icon: <Receipt size={20} /> },
        ...
        <div className={`h-16 flex items-center ${isCollapsed ? "justify-center" : "px-6 justify-between"} border-b border-slate-100`}>
          <div className="flex items-center gap-3 overflow-hidden whitespace-nowrap">
            ...
            {(!isCollapsed || isMobileOpen) && (
              <span className="font-bold text-lg tracking-tight text-slate-800">Shop Ops</span>
            )}
          </div>
          
          {/* mobile close button */}
          <button onClick={() => setIsMobileOpen(false)} className="md:hidden text-slate-400 hover:text-red-500">
            <X size={20} />
          </button>
        </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                `}
              >
                <div className="shrink-0 transition-colors duration-200">{item.icon}</div>
