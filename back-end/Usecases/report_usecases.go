...
var (
	ErrReportNotAvailableForFreeTier = errors.New("CSV export is only available for premium tier")
...
// ExportReportToCSV exports a report to CSV format, checking premium tier
func (u *ReportUsecases) ExportReportToCSV(businessID primitive.ObjectID, reportType string, dateRange Domain.DateRange, groupBy Domain.GroupBy) (string, error) {
	// Check business tier
	business, err := u.businessRepo.FindByID(businessID.Hex())
	if err != nil {
	...
	if business.Tier != Domain.TierPremium {
		return "", ErrReportNotAvailableForFreeTier
	}
	...
	switch reportType {
	case "sales":
		report, err := u.GenerateSalesReport(businessID, dateRange, groupBy)
		if err != nil {
			return "", err
		}
		csvData = report.ToCSV()
	case "expenses":
		report, err := u.GenerateExpenseReport(businessID, dateRange, groupBy)
		if err != nil {
			return "", err
		}
		csvData = report.ToCSV()
	case "profit":
		report, err := u.GenerateProfitReport(businessID, dateRange, groupBy)
		if err != nil {
			return "", err
		}
		csvData = report.ToCSV()
	case "inventory":
		report, err := u.GenerateInventoryReport(businessID)
		if err != nil {
			return "", err
		}
		csvData = report.ToCSV()
